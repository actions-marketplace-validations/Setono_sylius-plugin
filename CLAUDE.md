...
Force-pushing `v2` triggers a Packagist **"blocked update to version v2"** email on every release — this is expected and safe to ignore. Packagist indexes the floating `v2` tag as an immutable stable version (`2`), so moving it trips Packagist's version-immutability guard and freezes that `2` version at the prior commit. It's harmless: composer consumers use `^2.0`, which always resolves to the highest bare-numeric tag, never the frozen `2`. **Do not** follow the email's advice to restore the `v2` tag to its previous commit — that would roll back action consumers pinned to `@v2`. (Silencing the email permanently would mean making the floating major a *branch* named `v2` instead of a tag — Packagist treats branches as mutable dev versions — but that's a deliberate architecture change, not a routine fix.)
